...
fn vectorized_equal_to<GroupColumnBuilder: GroupColumn>(
    ...
    group.bench_function(id, |b| {
        ...
        b.iter(|| {
            // Creating a new buffer is a must as `vectorized_equal_to` will modify the
            // input buffer and without rebuilding it all benchmarks after the first one
            // won't be meaningful.
            let mut equal_to_results_buffer =
                BooleanBufferBuilder::new(equal_to_results.len());
            for is_equal in &equal_to_results {
                equal_to_results_buffer.append(*is_equal);
            }
